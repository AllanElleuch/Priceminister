...
class toolKit:
    def __init__(self, dataframe=None,randomForest=True):
        ...
        # self.count_vect = CountVectorizer(analyzer='word',stop_words=frozenset(stopwords.words('french'))  ) #regarde mot qui existe
        ...
        # self.vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=0.8,min_df=1,stop_words='english',max_features=250)
...
# draw a graph with review length as x and helpfulness as y
    ...
    def entrainerModelRandomForest(self,parameter=['len']): #TODO : Faire cross validation
        ...
        if( False ):
            ...
            final = self.getFeatures(parameter)
    ...
    def clear(self):
        self.dataFrame=None
    def getFeatures(self,parameter):
        tfidf_matrix = self.tfidf_matrix
        new_column = self.dataFrame[parameter]
